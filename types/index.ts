/**
 * Single entry point for all types in the project.
 * DB-generated utility types are re-exported here so nothing imports from ./types directly.
 */
 ...
 * Derived from the generated DB type: `embedding` is stripped (never sent to the client),
 * and `set_id` is added (column not yet reflected in the generated types).
 * NOTE: category is NOT stored — derive it with difficultyToCategory(word.difficulty).
 */
export type Word = Omit<Tables<'words'>, 'embedding'> & {
 ...
 * Kept manual because Supabase generates all view columns as nullable;
 * this interface enforces stricter non-null guarantees on id/score/type/created_at.
...
export type UserWordStat = Tables<'user_word_stats'>;
 ...
 * Derived from the generated DB type with the internal `id` primary key stripped.
 */
export type GameConfig = Omit<Tables<'game_config'>, 'id'>;
